...
function doRectanglesOverlap(rect1, rect2) {
  ...
  return !(rect1.left >= rect2Right || rect1Right <= rect2.left || rect1.top >= rect2Bottom
...
function isCreditCardNumber(cnn) {
  const digits = cnn.toString().split('').map(Number);
  let sum = 0;
  let isSecond = false;
...
function isBracketsBalanced(str) {
  ...
  const inputBrackets = str.match(bracketPattern);

  if (!inputBrackets || inputBrackets.length === 0) {
    return true;
  }
...
function evaluateTicTacToePosition(position) {
  ...
  const colWinner = [0, 1, 2]
    .map((col) => {
      if (position[0][col] && position[0][col] === position[1][col]
